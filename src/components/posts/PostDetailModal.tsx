...
export function PostDetailModal({ posts, startIndex, isOpen, onClose, onPostUpdate, readOnly = false }: PostDetailModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`w-[95vw] max-w-4xl h-[92vh] p-0 ${hasMedia ? 'grid grid-cols-1 md:grid-cols-2' : ''} gap-0`}>
        <Button variant="ghost" size="icon" className="absolute top-2 right-2 z-20 text-foreground md:text-foreground bg-background/20 md:bg-transparent hover:bg-background/40" onClick={onClose}><X /></Button>
        {currentIndex > 0 && <Button variant="ghost" size="icon" className="absolute left-2 top-1/2 -translate-y-1/2 z-20 text-foreground bg-background/20 hover:bg-background/40" onClick={goToPrev}><ChevronLeft /></Button>}
        {currentIndex < posts.length - 1 && <Button variant="ghost" size="icon" className="absolute right-2 md:right-[50%] md:mr-2 top-1/2 -translate-y-1/2 z-20 text-foreground bg-background/20 hover:bg-background/40" onClick={goToNext}><ChevronRight /></Button>}
        ...
        <div className={`flex flex-col p-4 bg-card ${hasMedia ? 'md:rounded-r-lg' : 'rounded-lg'}`}>
          ...
          <div ref={commentListRef} className="flex-1 py-4 overflow-y-auto space-y-4">
            {currentPost.text && (
              <div className="text-sm">
                <p className="text-foreground">{currentPost.text}</p>
              </div>
            )}
          ...
          <div className="border-t pt-2 mt-auto">
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" onClick={handleLike} disabled={!session || readOnly}><Heart className={isLiked ? 'text-destructive fill-current' : ''} /></Button>
              <Button variant="ghost" size="icon" onClick={() => commentInputRef.current?.focus()} disabled={readOnly}><MessageCircle /></Button>
              <Button variant="ghost" size="icon" disabled={readOnly}><Share2 /></Button>
              <div className="flex-grow" />
              <Button variant="ghost" size="icon" onClick={handleSave} disabled={!session || readOnly}><Bookmark className={isSaved ? 'text-primary fill-current' : ''} /></Button>
            </div>
            <p className="text-sm font-semibold px-2 mt-1 text-foreground">{currentPost.likes.length} likes</p>
            <form onSubmit={handleComment} className="flex gap-2 mt-2 px-2">
              <Input ref={commentInputRef} placeholder="Add a comment..." value={newComment} onChange={(e) => setNewComment(e.target.value)} disabled={isCommenting || readOnly} />
              <Button type="submit" disabled={readOnly || isCommenting || !newComment.trim()}>
                {isCommenting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Post"}
