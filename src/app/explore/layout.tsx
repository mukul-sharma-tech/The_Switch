...
// import { Home, User, MessageCircle, LogOut, BrainCircuit } from 'lucide-react';
...
//     {href: '/taru', icon: BrainCircuit, label: 'Taru AI' } // Placeholder for SearchModal
//   ];
      ...
      <nav className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 w-[95%] max-w-md bg-card border rounded-2xl shadow-lg flex justify-around py-2 z-50 backdrop-blur-md">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="flex flex-col items-center text-muted-foreground hover:text-primary w-1/5"
          >
            <item.icon className="w-5 h-5 mb-0.5" />
            <span className="text-[11px]">{item.label}</span>
          </Link>
        ...
        {/* Mobile: Search */}
        <div className="w-1/5">
          <SearchModal variant="mobile" />
        </div>

        {/* Mobile: Upload */}
        <div className="w-1/5">
          <UploadModal variant="mobile" />
        </div>
        ...
        <form action="/api/auth/signout" method="POST" className="w-1/5">
          <button
            type="submit"
            className="flex flex-col items-center text-muted-foreground hover:text-primary w-full"
          >
            <LogOut className="w-5 h-5 mb-0.5" />
            <span className="text-[11px]">Logout</span>
          </button>
