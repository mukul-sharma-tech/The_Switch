...
//         {post.photo && <Image src={post.photo} alt="Post content" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>}
...
//   const [modalContent, setModalContent] = useState<{title: 'Followers' | 'Following', users: { _id: string; name: string; username: string; profileImage?: string; }[]}|null>(null);
...
//   const { username } = React.use(params); // Unwrap params
...
//       } catch (error: unknown) {
//         const errorMessage = error instanceof Error ? error.message : "Could not load profile.";
...
//   useEffect(() => {
//     const fetchAllData = async () => {
//       // Use the unwrapped 'username' variable
//       if (!username) return;
//       setIsLoading(true);
...
//   if (!profile) {
//     // Use the unwrapped 'username' variable
//     return <div className="text-center mt-20 text-xl font-semibold">User &apos;{username}&apos; not found.</div>;
//   }
...
const PostCard = ({ post, onClick }: { post: Post, onClick: () => void }) => {
  ...
  return (
    <Card onClick={onClick} key={post._id} className="overflow-hidden group relative cursor-pointer">
      ...
      {/* THIS IS THE FIX: Added `relative` class to the parent of the Image with `fill` prop */}
